...
<template x-teleport="body">
    ...
    <div style="position:relative; background:white; border-radius:20px; width:100%; max-width:560px; max-height:92vh; overflow-y:auto; box-shadow:0 30px 80px rgba(0,0,0,0.25);"
        ...
        <div x-show="loading" style="padding:4rem; text-align:center;">
            <div style="width:40px; height:40px; border:3px solid #EDE7DC; border-top-color:#1E3A2F; border-radius:50%; animation:spin 0.7s linear infinite; margin:0 auto 1rem;"></div>
        ...
        <div x-show="!loading && booking && !booking.error">
            ...
            {{-- Header --}}
            <div style="padding:1.5rem 1.5rem 1.25rem; border-bottom:1px solid #F0EBE3; display:flex; align-items:center; justify-content:space-between; gap:1rem;">
                <div>
                    <div style="font-size:0.65rem; font-weight:700; color:#9CA3AF; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:0.2rem;">Booking Code</div>
                    <div style="font-size:1rem; font-weight:800; color:#1E3A2F; font-family:'DM Sans',sans-serif;" x-text="booking?.kode_booking"></div>
                </div>
            ...
            <div style="padding:1.5rem; display:flex; flex-direction:column; gap:1.5rem;">
                ...
                {{-- ── TRAVELER ── --}}
                <div>
                    <div style="font-size:0.65rem; font-weight:800; color:#9CA3AF; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:0.875rem;">Traveler</div>
                    <div style="display:flex; align-items:center; gap:1rem;">
                        ...
                        <div style="flex:1; min-width:0;">
                            <div style="display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap; margin-bottom:0.2rem;">
                                <div style="font-size:0.9rem; font-weight:700; color:#1E3A2F;" x-text="booking?.guest_name"></div>
                                <template x-if="booking?.guest_is_verified">
                                    <span style="font-size:0.65rem; font-weight:700; background:#EBF5EE; color:#2D5240; padding:0.15rem 0.5rem; border-radius:999px; letter-spacing:0.05em;">✓ VERIFIED</span>
                                </template>
                            </div>
                            ...
                            <template x-if="booking?.status === 'confirmed' || booking?.status === 'completed'">
                                <div style="display:flex; align-items:center; gap:0.4rem; margin-top:0.4rem;">
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#C4783A" stroke-width="2" stroke-linecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13 19.79 19.79 0 0 1 1.61 4.46 2 2 0 0 1 3.6 2.28h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.29 6.29l.95-.95a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                                    <span style="font-size:0.8rem; color:#C4783A; font-weight:600;" x-text="booking?.guest_phone ? booking.guest_phone : 'Belum ada nomor HP'"></span>
                                </div>
                            </template>
                            <template x-if="booking?.status !== 'confirmed' && booking?.status !== 'completed'">
                                <div style="display:flex; align-items:center; gap:0.4rem; margin-top:0.4rem;">
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" stroke-width="2" stroke-linecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13 19.79 19.79 0 0 1 1.61 4.46 2 2 0 0 1 3.6 2.28h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.29 6.29l.95-.95a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                                    <span style="font-size:0.8rem; color:#9CA3AF; font-style:italic;">(Muncul jika confirmed)</span>
                                </div>
                            </template>
                        </div>
                    </div>
                    <template x-if="booking?.notes_for_host">
                        <div style="margin-top:0.875rem; background:#FFFBF5; border:1px solid #F5E6CC; border-left:3px solid #C4783A; padding:0.75rem 0.875rem; border-radius:0 8px 8px 0;">
                            <div style="font-size:0.65rem; font-weight:700; color:#C4783A; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.3rem;">📝 Catatan / Special Request</div>
                            <div style="font-size:0.82rem; color:#3A3A3A; font-style:italic; line-height:1.6;" x-text="booking?.notes_for_host"></div>
                        </div>
                    </template>
                </div>
                ...
                {{-- ── EXPERIENCE ── --}}
                <div>
                    <div style="font-size:0.65rem; font-weight:800; color:#9CA3AF; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:0.875rem;">Detail Experience</div>
                    <div style="background:#F7F3ED; border-radius:12px; padding:1rem 1.125rem;">
                        <div style="font-size:0.9rem; font-weight:700; color:#1E3A2F; margin-bottom:0.75rem;" x-text="booking?.experience_title"></div>
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem;">
                            <div>
                                <div style="font-size:0.65rem; color:#9CA3AF; margin-bottom:0.2rem;">📅 Tanggal</div>
                                <div style="font-size:0.82rem; font-weight:600; color:#1E3A2F;" x-text="booking?.tanggal"></div>
                            </div>
                            <div x-show="booking?.jam">
                                <div style="font-size:0.65rem; color:#9CA3AF; margin-bottom:0.2rem;">🕐 Jam Mulai</div>
                                <div style="font-size:0.82rem; font-weight:600; color:#1E3A2F;" x-text="booking?.jam"></div>
                            </div>
                            <div x-show="booking?.jam_selesai">
                                <div style="font-size:0.65rem; color:#9CA3AF; margin-bottom:0.2rem;">🕓 Jam Selesai</div>
                                <div style="font-size:0.82rem; font-weight:600; color:#1E3A2F;" x-text="booking?.jam_selesai"></div>
                            </div>
                            <div>
                                <div style="font-size:0.65rem; color:#9CA3AF; margin-bottom:0.2rem;">👥 Peserta</div>
                                <div style="font-size:0.82rem; font-weight:600; color:#1E3A2F;">
                                    <span x-text="booking?.jumlah_peserta"></span> orang
                                    <span x-show="booking?.is_private" style="font-size:0.65rem; background:#FDF6EE; color:#C4783A; padding:0.1rem 0.4rem; border-radius:999px; margin-left:0.3rem; font-weight:700;">PRIVATE</span>
                                </div>
                            </div>
                        </div>
                        <template x-if="booking?.meeting_point">
                            <div style="margin-top:0.75rem; padding-top:0.75rem; border-top:1px solid #EDE7DC;">
                                <div style="font-size:0.65rem; color:#9CA3AF; margin-bottom:0.2rem;">📍 Meeting Point / Lokasi Jemput</div>
                                <div style="font-size:0.82rem; font-weight:500; color:#1E3A2F;" x-text="booking?.meeting_point"></div>
                            </div>
                        </template>
                    </div>
                </div>
                ...
                {{-- ── KEUANGAN ── --}}
                <div>
                    <div style="font-size:0.65rem; font-weight:800; color:#9CA3AF; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:0.875rem;">Rincian Keuangan</div>
                    <div style="border:1.5px solid #EDE7DC; border-radius:12px; overflow:hidden;">
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:0.65rem 1rem; border-bottom:1px solid #F0EBE3;">
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:0.65rem 1rem; border-bottom:1px solid #F0EBE3;">
                            <span style="font-size:0.82rem; color:#7A7A6E;">
                                ...
                                <span x-show="booking?.jumlah_peserta > 1" style="font-size:0.72rem; color:#9CA3AF;" x-text="'(' + booking?.jumlah_peserta + ' orang)'"></span>
                            </span>
                            <span style="font-size:0.82rem; font-weight:600; color:#1E3A2F;" x-text="booking?.total_harga"></span>
                        </div>
                        <template x-if="booking?.discount">
                            <div style="display:flex; justify-content:space-between; align-items:center; padding:0.65rem 1rem; border-bottom:1px solid #F0EBE3; background:#F9FFF9;">
                                <span style="font-size:0.82rem; color:#2D5240;">
                                    Diskon
                                    <span x-show="booking?.coupon_code" style="font-size:0.7rem; background:#EBF5EE; color:#2D5240; padding:0.1rem 0.4rem; border-radius:4px; margin-left:0.3rem;" x-text="booking?.coupon_code"></span>
                                </span>
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:0.65rem 1rem; border-bottom:1px solid #F0EBE3;">
                            ...
                            <span style="font-size:0.82rem; color:#9CA3AF;" x-text="'- ' + booking?.platform_fee"></span>
                        </div>
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:0.875rem 1rem; background:linear-gradient(135deg, #1E3A2F, #2D5240);">
                            <div>
                                <div style="font-size:0.65rem; font-weight:700; color:rgba(255,255,255,0.7); text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.15rem;">Kamu Terima</div>
                                <div style="font-size:0.72rem; color:rgba(255,255,255,0.5);">Setelah platform fee</div>
                            </div>
                            <span style="font-size:1.15rem; font-weight:800; color:white;" x-text="booking?.host_earning"></span>
                        </div>
                    </div>
                </div>
