...
@section('content')
    ...
    <div style="background:#F7F3ED; min-height:100vh; padding-bottom:4rem;">
        ...
        <div style="max-width:800px; margin:0 auto; padding:2.5rem 1.25rem 2rem;">
                ...
                @foreach($steps as $i => $step)
                    <div style="display:flex; align-items:center;">
                        <div style="display:flex; flex-direction:column; align-items:center; gap:0.35rem;">
                            <div
                                style="width:28px; height:28px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.75rem; font-weight:600; flex-shrink:0;
                                    {{ $step['num'] === 2 ? 'background:#1E3A2F; color:white;' : 'background:#E8E4DC; color:#7A7A6E;' }}">
                                {{ $step['num'] }}
                            </div>
                            <span class="checkout-step-label"
                                style="font-size:0.7rem; font-weight:{{ $step['num'] === 2 ? '600' : '400' }}; color:{{ $step['num'] === 2 ? '#1E3A2F' : '#9CA3AF' }}; white-space:nowrap;">
                                {{ $step['label'] }}
                            </span>
                        </div>
                        @if(!$loop->last)
            ...
            @if(session('error'))
                <div
                    style="background:#FEF2F2; border:1px solid #FECACA; color:#C0392B; padding:0.75rem 1rem; border-radius:8px; font-size:0.875rem; margin-bottom:1.5rem;">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="checkout-main-grid">
                ...
                <div>
                    <h2
                        style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.75rem; font-weight:500; color:#1E3A2F; margin-bottom:1.25rem;">
                        Booking Summary
                    </h2>
                    ...
                    <div style="background:white; border-radius:16px; border:1.5px solid #EDE7DC; overflow:hidden;">
                        ...
                        {{-- Experience Info --}}
                        <div
                            style="padding:1.25rem; display:flex; gap:1rem; align-items:flex-start; border-bottom:1px solid #EDE7DC;">
                            <div style="width:72px; height:72px; border-radius:10px; overflow:hidden; flex-shrink:0;">
                                ...
                                @else
                                    <div style="width:100%; height:100%; background:linear-gradient(135deg,#2D5240,#C4A882);">
                                    </div>
                                @endif
                            </div>
                            <div>
                                <div
                                    style="font-size:0.65rem; font-weight:700; color:#C4783A; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.25rem;">
                                    {{ strtoupper($experience->kategori?->getNama($locale) ?? 'Experience') }}
                                </div>
                                <div
                                    style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.05rem; font-weight:500; color:#1E3A2F; line-height:1.3; margin-bottom:0.25rem;">
                        ...
                        {{-- Date, Time, Guests --}}
                        <div
                            style="padding:1.25rem; display:grid; grid-template-columns:1fr 1fr; gap:1rem; border-bottom:1px solid #EDE7DC;">
                            <div>
                                <div
                                    style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.3rem;">
                                    Date</div>
                            ...
                            <div>
                                <div
                                    style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.3rem;">
                                    Time</div>
                                <div style="font-size:0.875rem; font-weight:500; color:#1E3A2F;">{{ $jamMulai }} –
                                    {{ $jamSelesai }} WITA</div>
                            </div>
                            <div>
                                <div
                                    style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.3rem;">
                                    Guests</div>
                                <div style="font-size:0.875rem; font-weight:500; color:#1E3A2F;">{{ $guests }}
                                    {{ $guests === 1 ? 'person' : 'people' }}</div>
                            </div>
                            <div>
                                <div
                                    style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.3rem;">
                                ...
                                <div style="font-size:0.875rem; font-weight:500; color:#1E3A2F;">
                                    {{ $experience->is_indoor ? 'Indoor' : 'Outdoor' }}</div>
                            </div>
                        </div>

                        {{-- Price Breakdown --}}
                        <div style="padding:1.25rem; border-bottom:1px solid #EDE7DC;" x-data="{
                                couponCode: '',
                                applying: false,
                                appliedCode: null,
                                discount: 0,
                            ...
                            <div style="margin-bottom:0.75rem;">
                                ...
                                <template x-if="appliedCode">
                                    <div
                                        style="display:flex; align-items:center; justify-content:space-between; padding:0.5rem 0.85rem; background:#EBF5EE; border:1px solid #B8DFC8; border-radius:8px;">
                                        <span style="font-size:0.8rem; color:#166534; font-weight:600;"
                                            x-text="'✓ ' + appliedCode"></span>
                                        <button type="button" @click="removeCoupon()"
                                            style="background:none; border:none; color:#166534; font-size:0.75rem; text-decoration:underline; cursor:pointer;">Remove</button>
                                    </div>
                                </template>
                                <p x-show="message && !appliedCode" x-text="message"
                                    style="font-size:0.75rem; color:#C0392B; margin-top:0.4rem;"></p>
                            </div>

                            <div x-show="discount > 0"
                                style="display:flex; justify-content:space-between; font-size:0.85rem; color:#166534; margin-bottom:0.75rem;">
                                <span>Diskon kupon</span>
                                <span x-text="'- Rp ' + discount.toLocaleString('id-ID')"></span>
                            </div>

                            <div
                                style="display:flex; justify-content:space-between; font-size:1rem; font-weight:700; color:#1E3A2F; padding-top:0.75rem; border-top:1px solid #EDE7DC;">
                                <span>Total</span>
                        ...
                        <div style="padding:1rem 1.25rem; background:#F7F3ED; border-bottom:1px solid #EDE7DC;">
                            <div
                                ...
                                Booking Code</div>
                            <div
                                style="font-size:0.875rem; font-weight:600; color:#1E3A2F; font-family:'DM Sans',sans-serif;">
                        ...
                        <div style="padding:1rem 1.25rem;" x-data="{ open: false }">
                            <button @click="open = !open"
                                ...
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7A7A6E"
                                    stroke-width="2" :style="open ? 'transform:rotate(180deg)' : ''"
                                    style="transition:transform 0.2s;">
                                    <path d="m6 9 6 6 6-6" />
                                </svg>
                            </button>
                            <div x-show="open" x-transition
                                style="margin-top:0.75rem; font-size:0.82rem; color:#7A7A6E; line-height:1.6;">
                    ...
                    <div style="text-align:center; margin-top:1.25rem;">
                        <div
                            ...
                            Secure Payment Via</div>
                        <div
                            style="display:flex; align-items:center; justify-content:center; gap:0.75rem; color:#7A7A6E; font-size:0.85rem; font-weight:600;">
                            <span>Midtrans</span>
                    ...
                    <div style="background:white; border-radius:16px; border:1.5px solid #EDE7DC; padding:1.75rem;">
                        ...
                        <form id="checkout-form" method="POST" action="{{ route('checkout.store', $experience->slug) }}">
                            ...
                            <div class="checkout-name-email-grid">
                                <div>
                                    <label
                                        style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; display:block; margin-bottom:0.4rem;">Full
                                        Name</label>
                                ...
                                <div>
                                    <label
                                        style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; display:block; margin-bottom:0.4rem;">Email
                                        Address</label>
                            ...
                            <div style="margin-bottom:1rem;">
                                <label
                                    style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; display:block; margin-bottom:0.4rem;">Phone
                                    Number</label>
                                <div style="display:flex; border:1.5px solid #E2DDD5; border-radius:8px; overflow:hidden;"
                                    ...
                                    <div
                                        style="padding:0.75rem 1rem; background:#F7F3ED; font-size:0.875rem; color:#1E3A2F; font-weight:500; border-right:1.5px solid #E2DDD5; white-space:nowrap;">
                                        +62
                                    </div>
                            ...
                            <div style="margin-bottom:1.5rem;">
                                <label
                                    style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; display:block; margin-bottom:0.4rem;">
                            ...
                            <div style="margin-bottom:1.5rem;">
                                <label style="display:flex; align-items:flex-start; gap:0.6rem; cursor:pointer;">
                                    <input type="checkbox" name="agree_terms" value="1"
                                        style="margin-top:2px; flex-shrink:0; accent-color:#1E3A2F;">
                                    <span style="font-size:0.82rem; color:#4A4A4A; line-height:1.5;">
                                        ...
                                        and
                                        <a href="#" style="color:#1E3A2F; text-decoration:underline;">Cancellation
                                            Policy</a>.
                                    </span>
                                </label>
                                @error('agree_terms')
                            ...
                            <button type="submit"
                                ...
                                onmouseover="this.style.background='#2D4A32'" onmouseout="this.style.background='#1E3A2F'">
                                Proceed to Payment
                            ...
                            {{-- Security Badge --}}
                            <div
                                style="display:flex; align-items:center; justify-content:center; gap:1rem; margin-top:1rem; font-size:0.75rem; color:#9CA3AF;">
