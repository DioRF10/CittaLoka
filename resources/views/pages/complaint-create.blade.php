...
@section('title', 'Ajukan Complaint — CittaLoka')
...
@section('content')
    ...
    @php
        ...
        $categories = [
            'no_show' => 'Host Tidak Hadir (No-Show)',
            'not_as_described' => 'Experience Tidak Sesuai Deskripsi',
    ...
    <div style="background:#FAFAF8; min-height:100vh; padding-bottom:5rem;">
        <div style="max-width:640px; margin:0 auto; padding:3rem 2rem 0;">
            ...
            <a href="{{ route('bookings.show', $booking->kode_booking) }}"
                style="font-size:0.8rem; color:#7A7A6E; text-decoration:none; display:inline-flex; align-items:center; gap:0.3rem; margin-bottom:1.5rem;">
                ← Kembali ke Booking Detail
            </a>

            <h1
                style="font-family:'Cormorant Garamond',Georgia,serif; font-size:2rem; font-weight:500; color:#1E3A2F; margin-bottom:0.5rem;">
                Ajukan Complaint
            </h1>
            <p style="font-size:0.9rem; color:#7A7A6E; margin-bottom:1.75rem;">
                Tell us what happened. The CittaLoka team will review and get back to you.
            </p>

            {{-- Card experience --}}
            <div
                style="display:flex; align-items:center; gap:0.85rem; background:white; border:1.5px solid #EDE7DC; border-radius:12px; padding:0.85rem 1rem; margin-bottom:1.75rem;">
                @if($cover)
                    <img src="{{ $cover->url }}" alt=""
                        style="width:52px; height:52px; border-radius:8px; object-fit:cover; flex-shrink:0;">
                @endif
                <div>
                    <div style="font-size:0.92rem; font-weight:500; color:#1E3A2F;">
                        {{ $booking->experience_title_snapshot }}</div>
                    <div style="font-size:0.75rem; color:#7A7A6E;">{{ $booking->host_name_snapshot }} ·
                        {{ \Carbon\Carbon::parse($booking->tanggal_experience)->translatedFormat('d F Y') }}</div>
                </div>
            </div>

            @if($errors->any())
                <div
                    style="background:#FEF2F2; border:1.5px solid #FECACA; color:#C0392B; padding:0.85rem 1rem; border-radius:10px; font-size:0.85rem; margin-bottom:1.5rem;">
                    {{ $errors->first() }}
                </div>
            @endif

            <form action="{{ route('complaints.store', $booking->kode_booking) }}" method="POST"
                ...
                @csrf
                <input type="file" name="photos[]" multiple accept="image/*" x-ref="photoInput" style="display:none;"
                    @change="addPhotos($event)">

                {{-- Kategori --}}
                <div style="margin-bottom:1.25rem;">
                    <label
                        style="display:block; font-size:0.85rem; font-weight:500; color:#1E3A2F; margin-bottom:0.5rem;">Kategori
                        complaint</label>
                ...
                <div style="margin-bottom:1.25rem;">
                    <label
                        style="display:block; font-size:0.85rem; font-weight:500; color:#1E3A2F; margin-bottom:0.5rem;">Ceritakan
                        detailnya</label>
                    <textarea name="description" rows="6" maxlength="2000" required
                        placeholder="Jelaskan apa yang terjadi, kapan, dan bagaimana kejadiannya..."
                ...
                <div style="margin-bottom:1.75rem;">
                    <label
                        style="display:block; font-size:0.85rem; font-weight:500; color:#1E3A2F; margin-bottom:0.5rem;">Lampirkan
                        bukti foto (opsional, maks 6)</label>
