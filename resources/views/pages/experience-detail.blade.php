...
@section('content')
    ...
    @if($isPreview)
        <div id="preview-banner" style="position:fixed; top:0; left:0; right:0; z-index:9999;
            ...
            <div style="display:flex; align-items:center; gap:0.75rem;">
                ...
                <div>
                    <div style="font-size:0.8rem; font-weight:700; letter-spacing:0.04em;">MODE PREVIEW — Tampilan Publik
                        Experience</div>
                    <div style="font-size:0.72rem; opacity:0.75;">Kamu sedang melihat tampilan yang dilihat traveler. Navigasi
                        dinonaktifkan.</div>
                </div>
            </div>
            <a href="{{ route('host.experiences.index') }}" style="background:rgba(255,255,255,0.15); border:1px solid rgba(255,255,255,0.3);
    ...
    @if($isPreview)
        ...
        <div id="preview-nav-toast" style="position:fixed; bottom:2rem; left:50%; transform:translateX(-50%);
    ...
    <div x-data="bookingWidget()" x-init="init()">
        ...
        <div style="max-width:1200px; margin:0 auto; padding:2rem;">
            ...
            {{-- ── Photo Gallery ── --}}
            <div
                style="display:grid; grid-template-columns:2fr 1fr; grid-template-rows:1fr 1fr; gap:0.75rem; height:480px; border-radius:16px; overflow:hidden; margin-bottom:2rem;">
                ...
                <div style="grid-row:1/3; background:#EDE7DC; overflow:hidden;">
                    ...
                    @else
                        <div
                            style="width:100%; height:100%; background:linear-gradient(135deg,#2D5240,#C4A882); display:flex; align-items:center; justify-content:center; font-size:4rem;">
                            🌿</div>
                    @endif
                </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 380px; gap:3rem; align-items:start;">
                ...
                <div>
                    ...
                    <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:1rem; margin-bottom:1rem;"
                        ...
                        data-auth="{{ auth()->check() ? '1' : '0' }}" data-login-url="{{ route('login') }}"
                        data-toggle-url="{{ route('wishlist.toggle') }}" data-experience-id="{{ $experience->id }}"
                        data-csrf="{{ csrf_token() }}">
                        <h1
                            style="font-family:'Cormorant Garamond',Georgia,serif; font-size:2.5rem; font-weight:500; color:#1E3A2F; line-height:1.2; margin:0;">
                            {{ $judul }}
                        </h1>
                        <button @click="
                            ...
                            <svg width="18" height="18" viewBox="0 0 24 24" :fill="wishlisted ? '#EF4444' : 'none'"
                                :stroke="wishlisted ? '#EF4444' : '#1E3A2F'" stroke-width="2" stroke-linecap="round"
                    ...
                    {{-- Meta --}}
                    <div
                        style="display:flex; align-items:center; gap:1.5rem; margin-bottom:1.5rem; font-size:0.875rem; color:#7A7A6E;">
                    ...
                    {{-- Host Card --}}
                    <div
                        style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.5rem; padding:1.25rem; background:#F7F3ED; border-radius:12px; border:1.5px solid #EDE7DC;">
                        <div style="display:flex; align-items:center; gap:1rem;">
                            ...
                            <div>
                                ...
                                <div
                                    style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.1rem; font-weight:500; color:#1E3A2F;">
                                    {{ $hostUser->name }}</div>
                                <div style="font-size:0.78rem; color:#7A7A6E;">
                                    {{ $host->bio ? Str::limit($host->bio, 50) : 'Experienced local host · Fluent in English' }}
                                </div>
                            </div>
                        </div>
                        <div style="display:flex; align-items:center; gap:0.6rem;"
                            ...
                            data-auth="{{ auth()->check() ? '1' : '0' }}" data-login-url="{{ route('login') }}"
                            data-follow-url="{{ route('hosts.follow-toggle') }}" data-host-id="{{ $host->id }}"
                            data-csrf="{{ csrf_token() }}">
                            <button @click="
                                ...
                                " class="btn-follow-host" :class="following ? 'is-following' : ''">
                    ...
                    {{-- About --}}
                    <h2
                        style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.5rem; font-weight:500; color:#1E3A2F; margin-bottom:1rem;">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1.5rem;">
                        @foreach($whatYouDo as $act)
                            <div style="padding:1rem; background:#F7F3ED; border-radius:10px; border:1.5px solid #EDE7DC;">
                                ...
                                <div style="font-size:0.875rem; font-weight:600; color:#1E3A2F; margin-bottom:0.3rem;">
                                    {{ $act['title'] ?? '' }}</div>
                    ...
                    <h2
                        style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.5rem; font-weight:500; color:#1E3A2F; margin-bottom:1rem;">
                        Inclusions &amp; Details</h2>
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:2rem; margin-bottom:1.5rem;">
                        <div>
                            <div
                                style="font-size:0.72rem; font-weight:600; color:#1E3A2F; text-transform:uppercase; letter-spacing:0.06em; margin-bottom:0.75rem;">
                                ✓ What's Included</div>
                            @foreach($includedItems as $item)
                                <div
                                    style="display:flex; align-items:flex-start; gap:0.5rem; margin-bottom:0.5rem; font-size:0.85rem; color:#4A4A4A;">
                        ...
                        <div>
                            <div
                                style="font-size:0.72rem; font-weight:600; color:#C0392B; text-transform:uppercase; letter-spacing:0.06em; margin-bottom:0.75rem;">
                    ...
                    {{-- Meeting Point --}}
                    <h2
                        style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.5rem; font-weight:500; color:#1E3A2F; margin-bottom:0.5rem;">
                        Meeting Point</h2>
                    <div
                        style="display:flex; align-items:flex-start; gap:0.5rem; font-size:0.875rem; color:#4A4A4A; margin-bottom:1rem;">
                        <span>📍</span>
                        <span>{{ $experience->meeting_point ?? $experience->alamat_lengkap ?? 'Location will be shared after booking' }}</span>
                    </div>
                    <div
                        style="background:#EDE7DC; border-radius:12px; height:200px; overflow:hidden; margin-bottom:1.5rem; border:1.5px solid #D4C4AC;">
                        ...
                        @else
                            <div
                                style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; flex-direction:column; color:#7A7A6E; gap:0.5rem;">
                    ...
                    <div id="reviews"
                        ...
                        <h2
                            style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.5rem; font-weight:500; color:#1E3A2F;">
                            Guest Reviews</h2>
                        <a href="{{ route('experiences.reviews', $experience->slug) }}"
                            style="font-size:0.8rem; color:#1E3A2F; text-decoration:underline; font-weight:500;">See all
                            {{ $approvedCount }} reviews</a>
                    </div>
                    <div style="display:flex; align-items:center; gap:2rem; margin-bottom:1.5rem;">
                        <div style="text-align:center;">
                                @if($approvedCount > 0)
                                    <div
                                        style="font-size:3rem; font-weight:300; color:#1E3A2F; font-family:'DM Sans',sans-serif; line-height:1;">
                                        {{ number_format($experience->rating_avg, 1) }}</div>
                                    <div style="color:#C4783A; font-size:1.1rem; margin:0.25rem 0;">
                                        @for($i = 0; $i < round($experience->rating_avg); $i++)★@endfor@for($i = round($experience->rating_avg); $i < 5; $i++)☆@endfor
                                    </div>
                                    <div style="font-size:0.75rem; color:#7A7A6E;">{{ $approvedCount }} reviews</div>
                                @else
                                    <div
                                        style="font-size:3rem; font-weight:300; color:#1E3A2F; font-family:'DM Sans',sans-serif; line-height:1;">0.0</div>
                                    <div style="color:#C4783A; font-size:1.1rem; margin:0.25rem 0;">☆☆☆☆☆</div>
                                    <div style="font-size:0.75rem; color:#7A7A6E;">0 reviews</div>
                                @endif
                            </div>
                        <div style="flex:1;">
                    ...
                    @if($reviews->isEmpty())
                        <div
                            style="background:#F7F3ED; border:1.5px solid #EDE7DC; border-radius:12px; padding:1rem 1.25rem; color:#7A7A6E; font-size:0.875rem;">
                            Belum ada review yang dipublikasikan untuk experience ini.
                        </div>
                    @else
                                ...
                                <div style="background:#F7F3ED; border-radius:12px; padding:1rem; border:1.5px solid #EDE7DC;">
                                    <div style="display:flex; align-items:center; gap:0.6rem; margin-bottom:0.75rem;">
                                        ...
                                        <div>
                                            ...
                                            <div style="font-size:0.72rem; color:#7A7A6E;">
                                                {{ $review->published_at?->translatedFormat('M Y') }}</div>
                                        </div>
                                    </div>
                ...
                <div style="position:sticky; top:2rem; align-self:start;">
                    <div
                        style="background:white; border-radius:16px; border:1.5px solid #EDE7DC; padding:1.75rem; box-shadow:0 4px 24px rgba(30,58,47,0.06); font-family:'DM Sans',sans-serif;">

                        {{-- Harga --}}
                        <div style="display:flex; align-items:baseline; gap:0.35rem; margin-bottom:0.25rem;">
                            <span
                                style="font-size:1.6rem; font-weight:700; color:#1E3A2F; letter-spacing:-0.02em;">{{ $harga }}</span>
                        ...
                        {{-- Input Group --}}
                        <div style="border:1.5px solid #E2DDD5; border-radius:10px; margin-bottom:1.25rem; position:relative;"
                            ...
                            <div style="position:relative;">
                                <button @click="showCalendar = !showCalendar"
                                    ...
                                    <div>
                                        <div
                                            style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.2rem;">
                                            Date</div>
                                ...
                                <div x-show="showCalendar" x-transition:enter="transition ease-out duration-150"
                                    ...
                                    <div
                                        ...
                                        <template x-for="d in ['Mo','Tu','We','Th','Fr','Sa','Su']" :key="d">
                                            <div style="text-align:center; font-size:0.65rem; color:#9CA3AF; font-weight:600; padding:2px 0;"
                                                x-text="d"></div>
                                        </template>
                                    </div>
                                    ...
                                    <div style="display:grid; grid-template-columns:repeat(7,1fr); gap:2px;">
                                        ...
                                        <template x-for="day in daysInMonth" :key="day">
                                            <button @click.stop="selectDate(day)"
                                                :disabled="!isAvailable(day) || isPast(day)" x-text="day"
                                    ...
                                    <div
                                        style="display:flex; gap:1rem; margin-top:0.75rem; font-size:0.65rem; color:#9CA3AF; justify-content:center;">
                                        <span style="display:flex; align-items:center; gap:0.25rem;">
                                            <span
                                                style="width:8px; height:8px; border-radius:2px; background:#EBF5EE; border:1px solid #B8DFC8; display:inline-block;"></span>
                                            Available
                                        </span>
                                        <span style="display:flex; align-items:center; gap:0.25rem;">
                                            <span
                                                style="width:8px; height:8px; border-radius:2px; background:#F5F5F5; display:inline-block;"></span>
                                            Unavailable
                                        </span>
                                        <span style="display:flex; align-items:center; gap:0.25rem;">
                                            <span
                                                style="width:8px; height:8px; border-radius:2px; background:#1E3A2F; display:inline-block;"></span>
                                            Selected
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div x-show="selectedDate || loadingTimes" x-transition:enter="transition ease-out duration-200"
                                x-transition:enter-start="opacity-0 -translate-y-2"
                                x-transition:enter-end="opacity-100 translate-y-0"
                                style="border-bottom:1.5px solid #E2DDD5;">
                                <div style="padding:0.85rem 1rem;">
                                    <div
                                        style="font-size:0.65rem; font-weight:700; color:#7A7A6E; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.5rem;">
                                        Time</div>

                                    {{-- Loading --}}
                                    <div x-show="loadingTimes"
                                        style="display:flex; align-items:center; gap:0.5rem; font-size:0.8rem; color:#9CA3AF; padding:0.25rem 0;">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            stroke-width="2" class="spin-icon">
                                    ...
                                    <div x-show="!loadingTimes && availableTimes.length > 0"
                                        ...
                                        <template x-for="t in availableTimes" :key="t.time">
                                            <button @click="if(t.available_slot > 0) selectedTime = t.time"
                                                ...
                                                <div x-text="t.time"
                                                    style="font-size:0.8rem; font-weight:600; font-family:'DM Sans',sans-serif; margin-bottom:0.1rem;">
                                                </div>
                                                <div x-text="t.available_slot > 0 ? t.available_slot + ' slots' : 'Full'"
                                                    style="font-size:0.65rem; font-family:'DM Sans',sans-serif; opacity:0.85;">
                                                </div>
                                            </button>
                                        </template>
                                    </div>
                            ...
                            <div style="position:relative;">
                                ...
                                <svg style="position:absolute; right:1rem; top:50%; transform:translateY(-50%); pointer-events:none;"
                        ...
                        @auth
                            ...
                            @else
                                ...
                                <button @click="bookNow()" :disabled="!selectedDate || !selectedTime"
                        ...
                        <button @click="toggleWishlist()"
                            ...
                            onmouseover="this.style.borderColor='#1E3A2F'" onmouseout="this.style.borderColor='#E2DDD5'">
                            <svg width="16" height="16" viewBox="0 0 24 24" :fill="inWishlist ? '#EF4444' : 'none'"
                                :stroke="inWishlist ? '#EF4444' : 'currentColor'" stroke-width="1.8" stroke-linecap="round"
                                stroke-linejoin="round">
                                <path
                                    d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
                            </svg>
                        ...
                        <p style="text-align:center; font-size:0.75rem; color:#9CA3AF; margin-bottom:1.25rem;">You won't be
                            charged yet</p>

                        {{-- Price Breakdown --}}
                        <div style="border-top:1px solid #EDE7DC; padding-top:1rem;">
                            <div
                                style="display:flex; justify-content:space-between; font-size:0.82rem; color:#6B7280; margin-bottom:0.5rem;">
                                <span x-text="`${priceFormatted} × ${guests} person${guests > 1 ? 's' : ''}`"></span>
                                <span x-text="subtotalFormatted" style="color:#4A4A4A;"></span>
                            </div>
                            <div
                                style="display:flex; justify-content:space-between; font-size:0.82rem; color:#6B7280; margin-bottom:0.85rem;">
                                <span>Service fee</span>
                                <span style="color:#4A4A4A;">Rp {{ number_format($serviceFee, 0, ',', '.') }}</span>
                            </div>
                            <div
                                style="display:flex; justify-content:space-between; font-size:0.95rem; font-weight:700; color:#1E3A2F; padding-top:0.85rem; border-top:1px solid #EDE7DC;">
                                <span>Total</span>
                        ...
                        {{-- Free Cancellation --}}
                        <div
                            style="display:flex; align-items:center; justify-content:center; gap:0.4rem; font-size:0.75rem; color:#2D5240; margin-top:1rem; padding-top:1rem; border-top:1px solid #EDE7DC;">
