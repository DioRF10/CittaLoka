...
<style>
    .ed-gallery-grid { display:grid; grid-template-columns:2fr 1fr; grid-template-rows:1fr 1fr; gap:0.75rem; height:480px; border-radius:16px; overflow:hidden; margin-bottom:2rem; }
    .ed-main-grid { display:grid; grid-template-columns:1fr 380px; gap:3rem; align-items:start; }
    .ed-2col-grid { display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1.5rem; }
    .ed-inclusions-grid { display:grid; grid-template-columns:1fr 1fr; gap:2rem; margin-bottom:1.5rem; }
    .ed-reviews-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:1rem; }

    @media (max-width: 768px) {
        .ed-gallery-grid { grid-template-columns:1fr; grid-template-rows:240px; height:240px; }
        .ed-gallery-grid > :nth-child(2) { display:none; }
        .ed-gallery-grid > :nth-child(3) { display:none; }
        .ed-main-grid { grid-template-columns:1fr; gap:2rem; }
        .ed-2col-grid { grid-template-columns:1fr; }
        .ed-inclusions-grid { grid-template-columns:1fr; gap:1.5rem; }
    ...
    <div style="max-width:1200px; margin:0 auto; padding:2rem 1.25rem;">
        ...
        <div class="ed-main-grid">
            ...
            <div>
                        ...
                        @foreach([5,4,3,2,1] as $star)
                            <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.3rem;">
                                ...
                                <div style="flex:1; height:6px; background:#EDE7DC; border-radius:999px; overflow:hidden;">
                                    <div style="height:100%; background:#1E3A2F; border-radius:999px; width:{{ $ratingBreakdown[$star] ?? 0 }}%;"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                @if($reviews->isEmpty())
                    <div style="background:#F7F3ED; border:1.5px solid #EDE7DC; border-radius:12px; padding:1rem 1.25rem; color:#7A7A6E; font-size:0.875rem;">
                        Belum ada review yang dipublikasikan untuk experience ini.
                    </div>
                @else
                    <div class="ed-reviews-grid">
                        @foreach($reviews as $review)
                            <div style="background:#F7F3ED; border-radius:12px; padding:1rem; border:1.5px solid #EDE7DC;">
                                <div style="display:flex; align-items:center; gap:0.6rem; margin-bottom:0.75rem;">
                                    ...
                                    @endif
                                    <div>
                                        <div style="font-size:0.85rem; font-weight:500; color:#2C2C2C;">{{ $review->user->name ?? 'Guest' }}</div>
                                        <div style="font-size:0.72rem; color:#7A7A6E;">{{ $review->published_at?->translatedFormat('M Y') ?? 'Baru saja' }}</div>
                                    </div>
                                </div>
                                ...
                                @endif
                                @if($review->photos && $review->photos->isNotEmpty())
                                    <div style="display:flex; gap:0.4rem; flex-wrap:wrap; margin-top:0.6rem;">
                                        @foreach($review->photos as $photo)
                                            <img src="{{ $photo->url }}" alt="Review photo" style="width:56px; height:56px; object-fit:cover; border-radius:6px; border:1px solid #EDE7DC;">
                                        @endforeach
                                    </div>
                                @endif
                            </div>
