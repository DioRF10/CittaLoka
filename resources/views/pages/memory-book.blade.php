...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;0,600;1,300;1,400;1,500&family=DM+Sans:wght@300;400;500;600&family=Caveat:wght@400;500;600&display=swap"
        rel="stylesheet">
...
<body>
    ...
    @if ($memoryBook->photos->count() > 0)
        <section class="section gallery-section" x-data="galleryScroll()">
                ...
                <div class="gallery-scroll-wrapper">
                    ...
                    <div class="gallery-scroll-track" x-ref="scrollTrack">
                        @foreach ($memoryBook->photos as $photo)
                            <div class="gallery-photo-item" x-on:click="openLightbox('{{ $photo->url }}')">
                                <img src="{{ $photo->url }}" alt="Memory foto {{ $loop->iteration }}" loading="lazy">
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            {{-- Lightbox --}}
            <div class="lightbox" x-show="lightboxOpen" x-on:click.self="lightboxOpen = false"
        ...
        <div class="container">
            <div class="bottom-grid">
                ...
                <div>
                    <div class="cta-card">
                        <div class="cta-icon">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#C4783A"
                    ...
                    @if ($booking->status === 'completed')
                        <div style="margin-top:1rem;">
                            <p style="font-size:0.75rem; color:var(--gray-text); text-align:center; margin-bottom:0.75rem;">
                                How was your experience?</p>
                            <p style="font-size:0.8rem; color:var(--gray-text); text-align:center; margin-bottom:0.75rem;">
                                Yuk tulis review untuk {{ $booking->host_name_snapshot }}</p>
    ...
    <div class="footer-note">
        <p>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C4783A" stroke-width="2"
                stroke-linecap="round" stroke-linejoin="round">
                <path
                    d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
            </svg>
            Dibuat dengan penuh cinta oleh <strong
                style="color:var(--green-dark); margin: 0 0.25rem;">{{ $booking->host_name_snapshot }}</strong> untuk
            <strong style="color:var(--green-dark); margin: 0 0.25rem;">{{ $booking->user->name }}</strong>
        </p>
    </div>
